...
export function useScandocAuthentication() {
  ...
  const authenticate = useCallback(
    async (baseUrl: string, creds: ScandocAuthRequest) => {
      setIsLoading(true);
      setError(null);
      credsRef.current = creds;
      try {
        const result = await authenticateScandoc(baseUrl, creds);
        setData(result);
  ...
  const refresh = useCallback(
    async (baseUrl: string) => {
      ...
      setError(null);
      try {
        const result = await refreshScandocAuth(baseUrl, refreshToken);
        setData(result);
        const accessToken = extractAccessToken(result);
